...
[
    ...
    if test "$GDAL_CONFIG" != "no"; then
        ...
        case "$host" in
            *mingw* | *msys*)
                echo "Removing .lib suffix from GDAL LDFLAGS and DEP_LDFLAGS"
                GDAL_LDFLAGS=`echo "$GDAL_LDFLAGS" | sed -e 's/\.lib//g'`
                GDAL_DEP_LDFLAGS=`echo "$GDAL_DEP_LDFLAGS" | sed -e 's/\.lib//g'`
            ;;
        esac
