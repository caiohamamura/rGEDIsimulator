...
float *uncompressPhotons(float *wave,dataStruct *data,photonStruct *photonCount,noisePar *noise,gediIOstruct *gediIO)
{
  ...
  ASSIGN_CHECKNULL_RETNULL(corrWave,crossCorrelateTime(photWave,data->res,data->nBins,gediIO->pulse,gediIO->pRes,gediIO->pclSwidth));
...
float *crossCorrelateTime(float *photWave,float res,int nBins,pulseStruct *pulse,float pRes,float pclSwidth)
{
  ...
  if(pulse->pclSmoo==NULL){
    if(pclSwidth>TOL){
      ASSIGN_CHECKNULL_RETNULL(pulse->pclSmoo,smooth(pclSwidth,pulse->nBins,pulse->resamp,res));
    }else             pulse->pclSmoo=pulse->resamp;
  }

  /*smooth waveform if needed*/
  if(pclSwidth>TOL){
    ASSIGN_CHECKNULL_RETNULL(smooWave,smooth(pclSwidth,nBins,photWave,res));
...
float **countPhotons(float *denoised,dataStruct *data,photonStruct *photonCount,int *nPhot,denPar *den,noisePar *noise,char pcl)
{
  ...
  photThresh=frand();
  ASSIGN_CHECKINT_RETNULL(nPhotons,(int)pickArrayElement(photThresh,photonCount->prob,photonCount->pBins,1));
