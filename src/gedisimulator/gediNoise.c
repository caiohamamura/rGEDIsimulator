...
int addNoise(dataStruct *data,noisePar *gNoise,float fSigma,float pSigma,float res,float rhoc,float rhog)
{
  ...
  if(gNoise->missGround){        /*Delete all signal beneath ground peak*/
    ...
    ISINTRETINT(applyLinkNoise(data,data->wave[data->useType],gNoise,res,rhoc,rhog,tempWave));
...
int applyLinkNoise(dataStruct *data,float *wave,noisePar *gNoise,float res,float rhoc,float rhog,float *tempWave)
{
  int i=0;
  float thisSig=0;
  float sigScale=0;
