...
int main(int argc,char **argv)
{
  ...
  ASSIGN_CHECKFLT_RETONE(dimage->noise.linkSig,setNoiseSigma(&dimage->noise,dimage->gediIO.linkPsig,dimage->gediIO.linkFsig,rhoC,rhoG));
  ...
  for(i=0;i<dimage->gediIO.nFiles;i++){
    ...
    if(dimage->readL2)ISINTRETONE(setL2ground(data,i,dimage));
    ...
    if(data->usable){
      ...
      if(dimage->renoiseWave)ISINTRETONE(modifyTruth(data,&dimage->noise));
      if(dimage->changeGrRho)ISINTRETONE(modifyGroundRho(data,dimage->scaleRhoVrhoG));
      ...
      if(!dimage->onlySNR){
        ...
        if(denoised&&checkUsable(denoised,data->nBins)){
          ...
          if(!dimage->ice2){
            ...
            /*Gaussian fit*/
            if(dimage->noRHgauss==0)ASSIGN_CHECKNULL_RETINT(processed, processFloWave(denoised,data->nBins,dimage->gediIO.gFit,1.0));
            ...
            /*write results*/
            if(dimage->readBinLVIS||dimage->readHDFlvis||dimage->readHDFgedi){ISINTRETINT(writeResults(data,dimage,metric,i,denoised,processed,dimage->gediIO.inList[0]));}
            else                                                             {ISINTRETINT(writeResults(data,dimage,metric,i,denoised,processed,dimage->gediIO.inList[i]));}
          }else{  /*ICESat-2 mode*/
        ...
        ISINTRETINT(calculateSNR(dimage,data,i));
      }/*metrics or SNR if*/
    }/*is the data usable*/
  ...
  if(!dimage->ice2&&!dimage->onlySNR)msgf("Written to %s.metric.txt\n",dimage->outRoot);
  if(dimage->onlySNR)ISINTRETINT(writeSNR(dimage->outRoot,dimage->snr));
  #ifdef USEPHOTON
  if(dimage->ice2)msgf("Written to %s\n",dimage->photonCount.outNamen);
  #endif
...
int writeSNR(char *outRoot,snrStruct *snr)
{
  int i=0,j=0,k=0;
  int wid=0;
  float sig=0;
...
int calculateSNR(control *dimage,dataStruct *data,int numb)
{
  ...
  /*allocate if needed*/
  if(dimage->snr==NULL)ISINTRETINT(allocateSNR(dimage));
  ...
  for(j=0;j<dimage->snr->nSig;j++){
    ...
    /*smooth waveform*/
    ASSIGN_CHECKNULL_RETINT(smoothed,smooth(sWidth,data->nBins,data->noised,data->res));
    if(!dimage->gediIO.pclPhoton&&!dimage->gediIO.pcl)ASSIGN_CHECKNULL_RETINT(smooGr,smooth(sWidth,data->nBins,data->ground[data->useType],data->res));
    ...
    for(i=0;i<dimage->snr->nMinWid;i++){
     ...
     #ifdef PHOTONCOUNT
     int designval = (int)dimage->photonCount.designval;
     #else
     int designval = (int)1;
     #endif
     dimage->snr->bSense[i][j][numb]=snrBeamSense(falsePosThresh,falseNegThresh,gWidth,data->wave[data->useType],\
                      ...
                      (int)(dimage->gediIO.pclPhoton+dimage->gediIO.photonWave)*designval);
...
int allocateSNR(control *dimage)
{
  int i=0,j=0;
  ...
  if(!(dimage->snr->linkM=(float ***)calloc(dimage->snr->nMinWid,sizeof(float **)))){
    errorf("error in snr linkM allocation.\n");
    return(-1);
  }
  if(!(dimage->snr->bSense=(float ***)calloc(dimage->snr->nMinWid,sizeof(float **)))){
    errorf("error in snr bSense allocation.\n");
    return(-1);
  }

  for(i=0;i<dimage->snr->nMinWid;i++){
    ASSIGN_CHECKNULL_RETINT(dimage->snr->linkM[i],fFalloc(dimage->snr->nSig,"snr linkM",i+1));
    ASSIGN_CHECKNULL_RETINT(dimage->snr->bSense[i],fFalloc(dimage->snr->nSig,"snr bSense",i+1));

    for(j=0;j<dimage->snr->nSig;j++){
      ASSIGN_CHECKNULL_RETINT(dimage->snr->linkM[i][j],falloc(dimage->snr->nWaves,"snr linkM",j+1));
      ASSIGN_CHECKNULL_RETINT(dimage->snr->bSense[i][j],falloc(dimage->snr->nWaves,"snr bSense",j+1));
    }
  }

  ASSIGN_CHECKNULL_RETINT(dimage->snr->cov,falloc(dimage->snr->nWaves,"snr cov",0));
  ASSIGN_CHECKNULL_RETINT(dimage->snr->gWidth,falloc(dimage->snr->nWaves,"snr gWidth",0));
...
int findMetrics(metStruct *metric,float *gPar,int nGauss,float *denoised,float *wave,int nBins,double *z,control *dimage,dataStruct *data)
{
  ...
  if(dimage->gediIO.ground||data->demGround){
    if(!dimage->rhNoGround){
      if(dimage->noise.linkNoise)ASSIGN_CHECKNULL_RETINT(metric->rhReal,findRH(data->wave[data->useType],z,nBins,data->gElev,dimage->rhRes,&metric->nRH));  /*original was noiseless*/
      else                       ASSIGN_CHECKNULL_RETINT(metric->rhReal,findRH(denoised,z,nBins,data->gElev,dimage->rhRes,&metric->nRH));  /*origina was noisy*/
    }else{
      ASSIGN_CHECKNULL_RETINT(canWave,subtractGroundFromCan(data->wave[data->useType],data->ground[data->useType],nBins));
      ASSIGN_CHECKNULL_RETINT(metric->rhReal,findRH(canWave,z,nBins,data->gElev,dimage->rhRes,&metric->nRH));   /*canopy only*/
      TIDY(canWave);
    }
  }else{
  ...
  ASSIGN_CHECKFLT_RETINT(metric->FHD,foliageHeightDiversity(denoised,nBins));
  if(dimage->noCanopy==0){   /*more complex ones only if needed*/
    if(dimage->gediIO.ground)ASSIGN_CHECKNULL_RETINT(canProf,canProfile(data->wave[data->useType],data->ground[data->useType],nBins));
    ...
    ASSIGN_CHECKFLT_RETINT(metric->FHDcanH,foliageHeightDiversityHist(canProf,nBins,dimage->fhdHistRes));
    TIDY(canProf);
    /*from Gaussian removed canopy*/
    if(nGauss>0)ASSIGN_CHECKNULL_RETINT(canProf,subtractGaussFromCan(denoised,nBins,mu[gInd],A[gInd],sig[gInd],z));
    else        canProf=NULL;  /*no Gaussian ground estimate*/
    ASSIGN_CHECKFLT_RETINT(metric->FHDcanGauss,foliageHeightDiversity(canProf,nBins));
    ASSIGN_CHECKFLT_RETINT(metric->FHDcanGhist,foliageHeightDiversityHist(canProf,nBins,dimage->fhdHistRes));
    TIDY(canProf);
  }

  /*lai profiles*/
  if(dimage->noCanopy==0){
    if(data->ground) {ASSIGN_CHECKNULL_RETINT(metric->tLAI,trueLAIprofile(data->wave[data->useType],data->ground[data->useType],z,nBins,dimage->laiRes,dimage->rhoRatio,data->gElev,dimage->maxLAIh,&metric->laiBins,dimage->gediIO.res));}
    else {ASSIGN_CHECKNULL_RETINT(metric->tLAI,trueLAIprofile(data->wave[data->useType],NULL,z,nBins,dimage->laiRes,dimage->rhoRatio,data->gElev,dimage->maxLAIh,&metric->laiBins,dimage->gediIO.res));}
    if(sig){
      ...
      ASSIGN_CHECKNULL_RETINT(metric->gLAI,trueLAIprofile(data->wave[data->useType],NULL,z,nBins,dimage->laiRes,dimage->rhoRatio,data->gElev,dimage->maxLAIh,&metric->laiBins,dimage->gediIO.res));
    }
    ASSIGN_CHECKNULL_RETINT(metric->hgLAI,halfEnergyLAIprofile(denoised,z,nBins,dimage->laiRes,dimage->rhoRatio,metric->gHeight,dimage->maxLAIh,&metric->laiBins,dimage->gediIO.res));
    ASSIGN_CHECKNULL_RETINT(metric->hiLAI,halfEnergyLAIprofile(denoised,z,nBins,dimage->laiRes,dimage->rhoRatio,metric->inflGround,dimage->maxLAIh,&metric->laiBins,dimage->gediIO.res));
    ASSIGN_CHECKNULL_RETINT(metric->hmLAI,halfEnergyLAIprofile(denoised,z,nBins,dimage->laiRes,dimage->rhoRatio,metric->maxGround,dimage->maxLAIh,&metric->laiBins,dimage->gediIO.res));
  }
  ...
  if(dimage->bayesGround){
    double *bayesGroundRes = bayesGround(wave,nBins,dimage,metric,z,data);
    ISNULLRETINT(bayesGroundRes);
    metric->bayGround = *bayesGroundRes;
...
float *halfEnergyLAIprofile(float *denoised,double *z,int nBins,float laiRes,float rhoRatio,double gElev,float maxLAIh,int *laiBins,float res)
{
  ...
  /*lai profile*/
  ASSIGN_CHECKNULL_RETNULL(LAI,findLAIprofile(canopy,totG,nBins,laiRes,laiBins,gElev,rhoRatio,z,maxLAIh,res));
...
float *gaussLAIprofile(float *denoised,double *z,int nBins,float laiRes,float rhoRatio,double gHeight,float maxLAIh,int *laiBins,float mu,float A,float sig,float res)
{
  ...
  /*lai profile*/
  ASSIGN_CHECKNULL_RETNULL(LAI,findLAIprofile(canopy,totG,nBins,laiRes,laiBins,gHeight,rhoRatio,z,maxLAIh,res));
...
float *trueLAIprofile(float *wave,float *ground,double *z,int nBins,float laiRes,float rhoRatio,double gElev,float maxLAIh,int *laiBins,float res)
{
  ...
  if(ground){
    ...
    /*lai profile*/
    ASSIGN_CHECKNULL_RETNULL(tLAI,findLAIprofile(canopy,totG,nBins,laiRes,laiBins,gElev,rhoRatio,z,maxLAIh,res));
...
float *findLAIprofile(float *canopy,float totG,int nBins,float laiRes,int *laiBins,double gElev,float rhoRatio,double *z,float maxLAIh,float res)
{
  ...
  *laiBins=(int)(maxLAIh/laiRes+0.5)+1;
  ...
  /*bin up waveform*/
  ASSIGN_CHECKNULL_RETNULL(binWave,falloc(*laiBins,"",0));
  ...
  ASSIGN_CHECKNULL_RETNULL(lngap,falloc((uint64_t)*laiBins,"apparent foliage profile",0));
  cumul=0.0;
  for(i=0;i<*laiBins;i++){
    cumul+=binWave[i];
    gap=1.0-(cumul/(totC+rhoRatio*totG));
  ...
  for (i=1;i<argc;i++){
    if (*argv[i]=='-'){
      if(!strncasecmp(argv[i],"-input",6)){
        ...
        ASSIGN_CHECKNULL_RETNULL(dimage->gediIO.inList[0],challoc((uint64_t)strlen(argv[++i])+30,"input name list",0));
